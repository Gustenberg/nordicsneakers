...
class StoreScraper:
    ...
    async def _scrape_nordic_sneakers(self, progress_callback=None) -> list[dict]:
        """Scrape produkter fra Nordic Sneakers sælger API."""
        ...
        async with httpx.AsyncClient(headers=headers, follow_redirects=True, timeout=30.0) as client:
            try:
                if progress_callback:
                    progress_callback("Henter Nordic Sneakers produkter...")

                response = await client.get(NORDIC_SNEAKERS_API_URL)

                if response.status_code == 200:
                    data = response.json()
                    # Produkter er pakket i "data" array
                    for product in data.get('data', []):
                        item = self._parse_nordic_product(product)
                        if item:
                            items.append(item)

                    logger.info(f"Nordic Sneakers scrape færdig: {len(items)} produkter")
                    if progress_callback:
    ...
    def save_to_database(self, items: list[dict], clear_existing: bool = False):
        """Gem scrapede produkter til databasen. Tilføjer kun nye elementer som standard."""
        if clear_existing:
        ...
        logger.info(f"Gemte {len(items)} produkter til database")
