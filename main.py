...
async def clear_wtb_data():
    ...
    update_scrape_status("wtb", count=0)
    invalidate_comparison_cache()
    logger.info("WTB data ryddet")
    return {"message": "WTB data ryddet"}


@app.delete("/api/data/products")
async def clear_products_data():
    """Ryd alle mine produkter."""
    database.clear_my_products()
    update_scrape_status("store", count=0)
    invalidate_comparison_cache()
    logger.info("Produkt data ryddet")
    return {"message": "Produkt data ryddet"}
